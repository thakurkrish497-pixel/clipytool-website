...
const p1 = require(path.join(rootDir, 'seo_data_part1.json'));
const p2 = require(path.join(rootDir, 'seo_data_part2.json'));
const p3 = require(path.join(rootDir, 'seo_data_part3.json'));
const data = { ...p1, ...p2, ...p3 };

const targets = [
    'video-cutter', 'extract-audio', 'gif-maker', 'video-filters', 
    'video-reverse', 'video-rotate', 'video-to-mp4', 'video-to-webm'
];

for (const folder of targets) {
  const seo = data[folder];
  if (!seo) {
      console.log(`No SEO data for ${folder}`);
      continue;
  }
  const filePath = path.join(rootDir, folder, 'index.html');
  ...
  // Inject before the module script
  const scriptTag = `<script type="module" src="/src/${folder}.js"></script>`;
  if (content.includes(scriptTag)) {
    content = content.replace(scriptTag, seoSection + '\n\n  ' + scriptTag);
    fs.writeFileSync(filePath, content);
    console.log(`Updated SEO for ${folder}`);
  } else {
    console.log(`Could not find script tag in ${folder}`);
  }
}
