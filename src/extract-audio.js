...
function setupExport() {
  dom.btnExport.addEventListener('click', processAudio);
}
...
async function processAudio() {
  ...
  try {
    ...
    ffmpeg.on('progress', ({ progress, time }) => {
      ...
      dom.exportBtnText.textContent = `Extracting... ${p}%${etaStr}`;
    });
    ...
    dom.exportBtnText.textContent = 'Extracting audio...';
    
    let args = ['-y', '-i', inputName];
    if (fmt === 'mp3') {
      args.push('-q:a', '0', '-map', 'a', outName);
    } else {
      args.push('-c:a', 'pcm_s16le', '-map', 'a', outName);
    }
    ...
    a.download = `${baseName}_audio.${fmt}`;
    a.click();
    ...
    setTimeout(() => {
      dom.exportBtnText.textContent = 'Extract Audio';
      dom.exportFill.style.width = '0%';
    ...
    dom.exportFill.classList.remove('active');
    state.isProcessing = false;
    setTimeout(() => { dom.exportBtnText.textContent = 'Extract Audio'; updateExportState(); }, 3000);
  }
}
