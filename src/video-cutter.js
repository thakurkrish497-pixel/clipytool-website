...
function updateExportState() {
  ...
  dom.exportNote.textContent = state.videoFile ? 'Ready to cut' : 'Upload a video to get started';
}
...
function setupExport() {
  dom.btnExport.addEventListener('click', processCut);
}
...
async function processCut() {
  ...
  try {
    ...
    ffmpeg.on('progress', ({ progress }) => {
      ...
      dom.exportBtnText.textContent = `Cutting... ${p}%${etaStr}`;
    });
    ...
    dom.exportBtnText.textContent = 'Cutting...';
    ...
    a.href = url;
    a.download = state.videoFile.name.replace(/\.[^/.]+$/, '') + '_cut.' + ext;
    a.click();

    dom.exportBtnText.textContent = 'Done ✓';
    dom.exportFill.style.width = '100%';
    dom.exportFill.style.background = 'rgba(245, 158, 11, 0.2)';

    await ffmpeg.deleteFile(inputName);
    await ffmpeg.deleteFile(outputName);

    setTimeout(() => {
      dom.exportBtnText.textContent = 'Cut Video';
      dom.exportFill.style.width = '0%';
    ...
    dom.exportFill.classList.remove('active');
    state.isProcessing = false;
    setTimeout(() => { dom.exportBtnText.textContent = 'Cut Video'; updateExportState(); }, 3000);
  }
}
