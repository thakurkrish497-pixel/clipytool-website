...
function loadVideo(file) {
  ...
  dom.videoPlayer.onloadedmetadata = () => {
    ...
    dom.fileDetailsVid.textContent = `${(file.size / 1024 / 1024).toFixed(1)} MB · ${dom.videoPlayer.duration.toFixed(1)}s`;
...
async function processVideo() {
  ...
  try {
    ...
    dom.exportBtnText.textContent = 'Reversing video...';
    
    let args = ['-i', inputName, '-vf', 'reverse'];
    
    if (dom.reverseMode.value === 'both') {
      args.push('-af', 'areverse');
    } else {
      args.push('-an'); // drop audio
    }
    
    args.push('-c:v', 'libx264', outName);
    ...
    setTimeout(() => {
      dom.exportBtnText.textContent = 'Reverse Video';
      dom.exportFill.style.width = '0%';
    ...
    dom.exportFill.classList.remove('active');
    state.isProcessing = false;
    setTimeout(() => { dom.exportBtnText.textContent = 'Reverse Video'; updateExportState(); }, 3000);
  }
}
