...
const dom = {
  ...
  webmQuality:   document.getElementById('webmQuality'),
...
async function processVideo() {
  ...
  try {
    ...
    const qual = dom.webmQuality.value;
    let bv = '1.5M';
    if (qual === 'high') bv = '2.5M';
    else if (qual === 'low') bv = '800k';
    ...
    a.download = `${baseName}_converted.webm`;
    a.click();
    ...
    setTimeout(() => {
      dom.exportBtnText.textContent = 'Convert to WebM';
      dom.exportFill.style.width = '0%';
    ...
    dom.exportFill.classList.remove('active');
    state.isProcessing = false;
    setTimeout(() => { dom.exportBtnText.textContent = 'Convert to WebM'; updateExportState(); }, 3000);
  }
}
